...
export default function AdminDashboard() {
    const { isLoggedIn, logout, content, updateHero, updateOptions, updateLeadCapture, updateServicesContent, updateValuesContent, updateProperties, updateKnowledgeBase, updateAssistantConfig, saveContent } = useAdmin();
    const [activeTab, setActiveTab] = useState<'hero' | 'options' | 'lead' | 'services' | 'values' | 'portfolio' | 'assistant'>('hero');
    ...
    const handleSaveProperty = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const newProp: Property = {
            ...
            oldPrice: formData.get('isPriceReduced') === 'on' ? (formData.get('oldPrice') as string || undefined) : undefined,
        ...
        <div className="fixed inset-0 z-[200] bg-slate-50 flex overflow-hidden">
            ...
            <aside className="w-64 bg-[#0A2540] text-white p-6 flex flex-col">
                ...
                <nav className="flex-1 space-y-2">
                    ...
                    <button
                        ...
                        className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${activeTab === 'lead' ? 'bg-[#009FE3] text-white' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
                    >
            ...
            <main className="flex-1 overflow-y-auto p-10 bg-white">
                ...
                {activeTab === 'lead' && (
                    <div className="max-w-3xl space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <section className="space-y-4">
                            <label className="block text-sm font-bold text-slate-700">Imagem Lateral (Formulário Leads)</label>
                            <input
                                type="file"
                                accept="image/png, image/jpeg, image/svg+xml"
                                onChange={async (e) => {
                                    ...
                                    if (file) {
                                        const reader = new FileReader();
                                        reader.onloadend = () => updateLeadCapture({ ...(content.leadCapture || {}), image: reader.result as string });
                                        reader.readAsDataURL(file);
                                    }
                                }}
                                className="w-full px-4 py-2 rounded-xl border border-slate-200 focus:ring-2 focus:ring-[#009FE3] file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-bold file:bg-[#009FE3] file:text-white hover:file:bg-[#0057A8] text-slate-500 text-sm cursor-pointer"
                            />
                            {content.leadCapture?.image && <img src={content.leadCapture.image} className="h-40 mt-4 object-cover rounded-xl shadow-md" alt="Lead Capture" />}
                        </section>
                    </div>
                )}
                ...
                {activeTab === 'services' && (
                    <div className="max-w-3xl space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <section className="space-y-4">
                            ...
                            <input
                                type="text"
                                value={content.servicesContent?.title || ''}
                                onChange={(e) => updateServicesContent({ ...content.servicesContent, title: e.target.value })}
                                className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-[#009FE3] outline-none transition-all"
                            />
                        </section>
                        <section className="space-y-4">
                            ...
                            <textarea
                                rows={3}
                                value={content.servicesContent?.description || ''}
                                onChange={(e) => updateServicesContent({ ...content.servicesContent, description: e.target.value })}
                                className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-[#009FE3] outline-none transition-all resize-none"
                            />
                        </section>
                        <section className="space-y-4">
                ...
                {activeTab === 'values' && (
                    <div className="max-w-3xl space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <section className="space-y-4">
                            ...
                            <input
                                type="text"
                                value={content.valuesContent?.title || ''}
                                onChange={(e) => updateValuesContent({ ...content.valuesContent, title: e.target.value })}
                                className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-[#009FE3] outline-none transition-all"
                            />
                        </section>
                        <section className="space-y-4">
                            <label className="block text-sm font-bold text-slate-700">Subtítulo (com gradiente)</label>
                            <input
                                type="text"
                                value={content.valuesContent?.subtitle || ''}
                                onChange={(e) => updateValuesContent({ ...content.valuesContent, subtitle: e.target.value })}
                                className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-[#009FE3] outline-none transition-all"
                            />
                        </section>
                        <section className="space-y-4">
                            <label className="block text-sm font-bold text-slate-700">Descrição Curta</label>
                            <textarea
                                rows={2}
                                value={content.valuesContent?.description || ''}
                                onChange={(e) => updateValuesContent({ ...content.valuesContent, description: e.target.value })}
                                className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-[#009FE3] outline-none transition-all resize-none"
                            />
                        </section>
                        <section className="space-y-4">
                            <label className="block text-sm font-bold text-slate-700">Citação Inspiradora / Propósito</label>
                            <textarea
                                rows={3}
                                value={content.valuesContent?.quote || ''}
                                onChange={(e) => updateValuesContent({ ...content.valuesContent, quote: e.target.value })}
                                className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-[#009FE3] outline-none transition-all resize-none"
                            />
                        </section>
                        <section className="space-y-4">
                ...
                {activeTab === 'assistant' && (
                    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        {/* Configurações Globais do Assistente */}
                        <div className="bg-slate-50 p-8 rounded-[2rem] border border-slate-200 shadow-sm space-y-6">
                            <h2 className="text-xl font-bold text-slate-800 border-b border-slate-200 pb-4">Configurações Gerais do Assistente</h2>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <label className="flex items-center gap-3 cursor-pointer">
                                        <div className="relative">
                                            <input
                                                type="checkbox"
                                                className="sr-only"
                                                checked={content.assistantConfig?.isActive ?? true}
                                                onChange={(e) => updateAssistantConfig({ ...(content.assistantConfig || { voiceGender: 'female', accent: 'pt-PT', whatsappNumber: '351939103038' }), isActive: e.target.checked })}
                                            />
                                            <div className={`block w-14 h-8 rounded-full transition-colors ${content.assistantConfig?.isActive !== false ? 'bg-[#009FE3]' : 'bg-slate-300'}`}></div>
                                            <div className={`dot absolute left-1 top-1 bg-white w-6 h-6 rounded-full transition-transform ${content.assistantConfig?.isActive !== false ? 'transform translate-x-6' : ''}`}></div>
                                        </div>
                                        <div className="font-bold text-slate-700">Assistente Ativado no Site</div>
                                    </label>
                                </div>

                                <div className="space-y-2">
                                    <label className="text-xs font-bold text-slate-500 uppercase">Número de WhatsApp (Captura)</label>
                                    <input
                                        type="text"
                                        value={content.assistantConfig?.whatsappNumber || '351939103038'}
                                        onChange={(e) => updateAssistantConfig({ ...(content.assistantConfig || { isActive: true, voiceGender: 'female', accent: 'pt-PT' }), whatsappNumber: e.target.value })}
                                        className="w-full px-4 py-3 rounded-xl border border-slate-200 outline-none focus:ring-2 focus:ring-[#009FE3]"
                                    />
                                </div>

                                <div className="space-y-2">
                                    <label className="text-xs font-bold text-slate-500 uppercase">Voz Padrão</label>
                                    <select
                                        value={content.assistantConfig?.voiceGender || 'female'}
                                        onChange={(e) => updateAssistantConfig({ ...(content.assistantConfig || { isActive: true, accent: 'pt-PT', whatsappNumber: '351939103038' }), voiceGender: e.target.value as 'female' | 'male' })}
                                        className="w-full px-4 py-3 rounded-xl border border-slate-200 outline-none focus:ring-2 focus:ring-[#009FE3]"
                                    >
                                ...
                                <div className="space-y-2">
                                    <label className="text-xs font-bold text-slate-500 uppercase">Sotaque / Idioma Nativo</label>
                                    <select
                                        value={content.assistantConfig?.accent || 'pt-PT'}
                                        onChange={(e) => updateAssistantConfig({ ...(content.assistantConfig || { isActive: true, voiceGender: 'female', whatsappNumber: '351939103038' }), accent: e.target.value as any })}
                                        className="w-full px-4 py-3 rounded-xl border border-slate-200 outline-none focus:ring-2 focus:ring-[#009FE3]"
                                    >
                    ...
                    <div className="fixed inset-0 z-[300] bg-[#0A2540]/80 backdrop-blur-md flex items-center justify-center p-6">
                        ...
                        >
                            ...
                            <form onSubmit={handleSaveProperty} className="space-y-6">
                                ...
                                <div className="p-5 rounded-2xl bg-red-50 border-2 border-red-200 shadow-sm transition-all hover:border-red-300">
                                    <label className="flex items-center gap-3 cursor-pointer">
                                        <input type="checkbox" name="isPriceReduced" defaultChecked={!!propModal.data?.oldPrice} className="w-5 h-5 accent-red-600 rounded cursor-pointer" />
                                        <span className="font-black text-red-600 uppercase tracking-widest">Destacar como Redução de Preço</span>
                                    </label>
                                    <div className="mt-4">
                                        <label className="text-xs font-bold text-red-500 uppercase">Preço Antigo (Aparecerá Riscado)</label>
                                        <input name="oldPrice" defaultValue={propModal.data?.oldPrice} placeholder="Ex: 50.000 €" className="w-full px-4 py-3 mt-1 rounded-xl border border-red-200 outline-none focus:ring-2 focus:ring-red-500 bg-white" />
                                    </div>
                                </div>
                                <div className="space-y-2">
