...
function sanitizeAIText(text: string): string {
  return text
    ...
    .replace(/\[!\[[^\]]*\]\([^)]+\)\]\(([^)]+)\)/g, (_match, url) => {
      return `[Confirmar no WhatsApp da Dilar](${url})`;
    });
}
          ...
          if (p.startsWith("[") && p.match(/\[([^\]]+)\]\(([^)]+)\)/)) {
            ...
            if (isWhatsApp && lead) {
              ...
              if (currentTextMatch) {
                ...
                url += `?text=${encodeURIComponent("Olá Dilar!" + leadDetails)}`;
              }
            }
...
export default function VirtualAssistant() {
  ...
  const [currentLang, setCurrentLang] = useState(content.assistantConfig?.accent || "pt-PT");
  ...
  const [voiceGender, setVoiceGender] = useState<"female" | "male">(content.assistantConfig?.voiceGender || "female");
  ...
  useEffect(() => {
    if (phase === "welcome") {
      setTimeout(() => {
        speak("Olá! 👋 Bem-vindo ao site da Dilar Horta! Sou a sua assistente virtual. Estou aqui para ajudar a encontrar o imóvel ideal ou agendar uma consultoria gratuita! 🏡");
      }, 300);
    }
  }, [phase, speak]);
  ...
  const handleQuickReply = useCallback(
    (qr: string) => {
      if (
        ...
        qr === "Falar com a Dilar"
      ) {
        let text = "Olá Dilar! Vim pelo Assistente Virtual do site e gostaria de falar consigo.";
        if (leadInfo.name) {
          text += `\n\n[Detalhes do Lead]`;
          text += `\nNome: ${leadInfo.name}`;
        ...
        const fallbackWhatsapp = AGENT_PROFILE.whatsapp || "351939103038";
        const targetWhatsapp = content.assistantConfig?.whatsappNumber || fallbackWhatsapp;

        const waUrl = `https://wa.me/${targetWhatsapp.replace(/\D/g, "")}?text=${encodeURIComponent(text)}`;
        window.open(waUrl, "_blank");
        pushAssistantMessage(
          `A abrir o WhatsApp da Dilar... 📱\nSe não abrir automaticamente, contacte: **${targetWhatsapp}**`,
  ...
  // ─────────────────────────────────────────────────────────────────────────
  if (content.assistantConfig?.isActive === false) return null;
  ...
  return (
    <>
      ...
      {/* ══════════════════════════════════════════════════════════════════ */}
      {/*  WELCOME — Animated intro screen with Dilar waving               */}
      {/* ══════════════════════════════════════════════════════════════════ */}
      <AnimatePresence>
        {phase === "welcome" && (
          <motion.div
      ...
      <AnimatePresence>
        {phase === "lead_form" && (
          <motion.div
            key="lead"
            className="fixed bottom-4 right-3 sm:bottom-6 sm:right-6 z-50 w-[calc(100vw-1.5rem)] sm:w-[340px]"
      ...
      <AnimatePresence>
        {phase === "chat" && (
          <motion.div
            ...
            className="fixed bottom-4 right-3 sm:bottom-6 sm:right-6 z-50 w-[calc(100vw-1.5rem)] sm:w-[340px] flex flex-col"
            style={{ maxHeight: minimized ? "auto" : "min(580px, calc(100vh-5rem))" }}
          >
            <div
              ...
              <div className="relative bg-gradient-to-r from-[#0057A8] via-[#006FCC] to-[#009FE3] px-4 py-3 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0 relative z-10">
                  <div className="flex items-center gap-2">
                    <p className="font-semibold text-white text-sm">Dilar Horta</p>
                  ...
                  >
                    ...
                    >
                      ...
                      {messages.length === 0 && !typing && (
                        ...
                        >
                          ...
                          <p className="text-xs text-slate-500 mt-0.5">Dilar Horta · RE/MAX Dinâmica</p>
                    ...
                    <div className="border-t border-slate-100 px-3 py-3 bg-white">
                      ...
                      <p className="text-center text-[9px] text-slate-400 mt-2">
                        Assistente Virtual IA · Dilar Horta RE/MAX · Respostas automáticas
                      </p>
                    </div>
                  </motion.div>
                )}
